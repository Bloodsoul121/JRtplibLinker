...
jlong initSendHandle_
        (JNIEnv *env, jobject type, jint _localport, jstring _desthost, jint _destport,
         jobject listener) {
    //创建Handle
...
jlong initReceiveAndSendHandle_
        (JNIEnv *env, jobject type, jstring _localhost, jint _localreceiveport,
         jint _localsendport,
         jstring _desthost,
...
//
//    //转换远端IP
//    const char *desthost = env->GetStringUTFChars(_desthost, nullptr);
//
//    //初始化发送端参数
...
//    handle->sender = (jlong) crtpSender;
    ...
    //转换本地IP
    const char *localhost = env->GetStringUTFChars(_localhost, nullptr);
...
jboolean sendByte_
        (JNIEnv *env, jobject type, jlong rtpHandler, jbyteArray _src, jint _byteLength,
    ...
    jbyte *src = env->GetByteArrayElements(_src, nullptr);
    bool sendResult;
    if (isRtpData) {
...
jboolean finiHandle_(JNIEnv *env, jobject type, jlong rtpHandler) {
    if (rtpHandler == 0) {
        return (jboolean) false;
    }
...
JNINativeMethod methods[] = {
        {"initSendHandle",           "(ILjava/lang/String;ILcom/banma/jrtpliblinker/helper/RtpListener;)J",                    (void *) initSendHandle_},
        {"initReceiveAndSendHandle", "(Ljava/lang/String;IILjava/lang/String;ILcom/banma/jrtpliblinker/helper/RtpListener;)J", (void *) initReceiveAndSendHandle_},
...
int registerNativeMethods(JNIEnv *pEnv) {
    jclass clazz = pEnv->FindClass("com/banma/jrtpliblinker/helper/RtpHandle");
    if (clazz == NULL) {
        logd("clazz == NULL ！");
    }
    //调用Env环境中的注册方法。
