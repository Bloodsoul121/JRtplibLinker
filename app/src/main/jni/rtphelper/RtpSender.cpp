...
CRTPSender::initParam(JavaVM *vm, JNIEnv *env, CRTPSender *sess, const char *host,
    ...
    logd("CRTPSender init status : %d", status);
    CheckError(status);
    if (status < 0) {
...
void CRTPSender::OnNewSource(RTPSourceData *srcdat) {
    ...
    if (srcdat->GetRTPDataAddress() != nullptr) {
        ...
        loge("%s CRTPSender RTP/RTCP. error \n", "OnNewSource");
        return;
    }
...
void CRTPSender::OnPollThreadStart(bool &stop) {
    ...
    int status = s_vm->AttachCurrentThread(&s_env, nullptr);
    if (status < 0) {
        s_isAttach = false;
        s_env = nullptr;
    ...
    s_isAttach = true;
    logd("CRTPSender AttachCurrentThread Ok!\n");
}
